...
OnBusAcquired(GDBusConnection* pConnection, const gchar* pName, gpointer userData)
{
	...
	g_signal_connect(sync_ipc_obj, "handle-add-on-demand-job", G_CALLBACK(sync_manager_add_on_demand_job), NULL);
	g_signal_connect(sync_ipc_obj, "handle-add-periodic-job", G_CALLBACK(sync_manager_add_periodic_job), NULL);
	g_signal_connect(sync_ipc_obj, "handle-add-data-change-job", G_CALLBACK(sync_manager_add_data_change_job), NULL);
	g_signal_connect(sync_ipc_obj, "handle-get-all-sync-jobs", G_CALLBACK(sync_manager_get_all_sync_jobs), NULL);
	g_signal_connect(sync_ipc_obj, "handle-remove-job", G_CALLBACK(sync_manager_remove_job), NULL);
